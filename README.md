...
2. **Install python packages** (run `pip install -r requirements.txt` in terminal, make sure to `cd` into the `lottery/` diretory first).*
3. **Install Gurobi software (if needed)**: MERIT uses [Gurobi software](https://www.gurobi.com/) to efficiently solve large linear programs. The installation in step (2) gives you a trial license that can be used to solve smaller problems (for example, selecting among a few hundred candidates.) If you run the code on a larger problem instance, you will likely encounter an error "Model too large for size-limited Gurobi license." If this occurs, please obtain Gurobi with a non-trial license. Academics can obtain a license to this software for free. Follow [instructions from Gurobi](https://www.gurobi.com/academia/academic-program-and-licenses/) to install and activate the license.
