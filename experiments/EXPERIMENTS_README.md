...
**Note on setup:** To run the `baseline_local_sensitivity` experiments that include baseline mechanisms, packages for solving linear programs are required. Please follow the instructions in [`baselines/README.md`](../baselines/README.md) to complete this setup. Running linear lottery + softmax only experiments (1-4) requires only the lighter setup described in the root [`README.md`](../README.md).
...
## Local Sensitivity (`local_sensitivity.py`)
Goal:
...
## Baseline Local Sensitivity (`baseline_local_sensitivity.py`)
Goal:
- Compare worst-case local sensitivity (single-review one-tick perturbation) across existing baseline partial lotteries and smooth mechanisms.

High-level setup:
- Mechanisms: `MERIT`, `Swiss NSF`, `Randomized Threshold`, `Linear Lottery`, `Softmax` (`L=1/r` for smooth mechanisms).
