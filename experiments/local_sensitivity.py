#!/usr/bin/env python3
"""Local sensitivity for smooth mechanisms under one-review one-tick edits.

Purpose:
- Estimate local sensitivity near the decision boundary for Linear Lottery and Softmax.

Method:
- For each dataset sample (Beta, NeurIPS, ICLR, Swiss NSF), perturb one review
  on rank-k or rank-(k+1) items by one tick (clipped to [0,1]).
- Sweep k settings and target L values.
- Report the largest local sensitivity found among candidate edits.

Outputs:
- `experiments/results/local_sensitivity_all.csv`
- `experiments/results/local_sensitivity_all_summary.csv`
- `experiments/figures/local_sensitivity_k10pct.pdf`
- `experiments/figures/local_sensitivity_k33pct.pdf`
- `experiments/figures/local_sensitivity_k50pct.pdf`
"""
...
from plot_results import plot_local_sensitivity_2x1
        ...
        if best is not None:
            ...
            if mech == "softmax":
                ...
                if selected_cand is not None:
                    ...
                    best["local_sensitivity"] = float(final_l1 / l11_delta_best) if l11_delta_best > 0 else np.nan
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Local sensitivity at k / k+1 under single-review one-tick perturbations")
    ...
    df = run(args)
    out_csv = os.path.join(args.output_dir, "local_sensitivity_all.csv")
    out_summary = os.path.join(args.output_dir, "local_sensitivity_all_summary.csv")
    ...
    if not df.empty:
        ...
        summary["ratio_to_L"] = summary["local_sensitivity"] / summary["L"]
        summary.to_csv(out_summary, index=False)

        for k_name in sorted(summary["k_name"].unique()):
            sub = summary[summary["k_name"] == k_name].copy()
            out_pdf = os.path.join(args.fig_dir, f"local_sensitivity_{k_name}.pdf")
            plot_local_sensitivity_2x1(sub, k_name=k_name, out_pdf=out_pdf)
            print(f"Saved {out_pdf}")
    else:
        pd.DataFrame(
            columns=[
                "dataset", "k_name", "L", "mechanism", "local_sensitivity",
