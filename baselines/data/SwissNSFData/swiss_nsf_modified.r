...
#'                                  }
get_mcmc_samples2 <- function(data, id_proposal, id_assessor,
                             grade_variable,
  ...
  samps1 <-
    run.jags(path_to_jags_model,
             ...
             sample = n_iter / thin, method = runjags_method, silent.jags = FALSE,
             thin=thin, progress.bar = "text")
  ...
  while (any(samps1$psrf$psrf[, "Point est."] > rhat_threshold) &
    ...
    counter <- counter + 1
    samps1 <- extend.jags(samps1, sample = n_iter / thin, method = runjags_method)
  }
